...
// A RawRowViewer can return a slice of float32 reflecting a row that is backed by the matrix
// data.
...
// A RawColViewer can return a slice of float32 reflecting a column that is backed by the matrix
// data.
...
func Col(dst []float32, j int, a Matrix) []float32 {
	...
	if rm, ok := aU.(RawMatrixer); ok {
		...
		blas32.Copy(blas32.Vector{N: r, Inc: m.Stride, Data: m.Data[j:]},
			blas32.Vector{N: r, Inc: 1, Data: dst},
		)
		return dst
	}
	for i := 0; i < r; i++ {
...
// slice is nil in which case a new slice is first allocated.
func Row32(dst []float32, i int, a Matrix) []float32 {
	r, c := a.Dims()
	if i < 0 || i >= r {
		panic(ErrColAccess)
	}
	...
	if rm, ok := aU.(RawMatrixer); ok {
		m := rm.RawMatrix()
		if aTrans {
			blas32.Copy(blas32.Vector{N: c, Inc: m.Stride, Data: m.Data[i:]},
				blas32.Vector{N: c, Inc: 1, Data: dst},
			)
			return dst
		}
...
// func Cond(a Matrix, norm float32) float32 {
// 	m, n := a.Dims()
// 	if m == 0 || n == 0 {
// 		panic(ErrZeroLength)
// 	}
...
// zero size.
// func Det(a Matrix) float32 {
// 	det, sign := LogDet(a)
// 	return float32(math.Exp(float64(det)) * float64(sign))
// }
...
func Dot(a, b Vector) float32 {
	...
	if arv, ok := a.(RawVectorer); ok {
		if brv, ok := b.(RawVectorer); ok {
			return blas32.Dot(arv.RawVector(), brv.RawVector())
		}
	}
	var sum float32
	for i := 0; i < la; i++ {
...
func EqualApprox(a, b Matrix, epsilon float32) bool {
	...
	if rma, ok := aU.(RawMatrixer); ok {
		if rmb, ok := bU.(RawMatrixer); ok {
			...
			if aTrans == bTrans {
				for i := 0; i < ra.Rows; i++ {
					for j := 0; j < ra.Cols; j++ {
						if !scalar.EqualWithinAbsOrRel(float64(ra.Data[i*ra.Stride+j]), float64(rb.Data[i*rb.Stride+j]), float64(epsilon), float64(epsilon)) {
							return false
						}
					}
				}
			...
			for i := 0; i < ra.Rows; i++ {
				for j := 0; j < ra.Cols; j++ {
					if !scalar.EqualWithinAbsOrRel(float64(ra.Data[i*ra.Stride+j]), float64(rb.Data[j*rb.Stride+i]), float64(epsilon), float64(epsilon)) {
						return false
					}
				}
			}
	...
	if ra, ok := aU.(*VecDense); ok {
		if rb, ok := bU.(*VecDense); ok {
			...
			// transposed or both not transposed (or have length 1).
			for i := 0; i < ra.mat.N; i++ {
				if !scalar.EqualWithinAbsOrRel(float64(ra.mat.Data[i*ra.mat.Inc]), float64(rb.mat.Data[i*rb.mat.Inc]), float64(epsilon), float64(epsilon)) {
					return false
				}
			}
			return true
	...
	for i := 0; i < ar; i++ {
		for j := 0; j < ac; j++ {
			if !scalar.EqualWithinAbsOrRel(float64(a.At(i, j)), float64(b.At(i, j)), float64(epsilon), float64(epsilon)) {
				return false
			}
		}
	}
...
// // has zero size.
// func LogDet(a Matrix) (det float32, sign float32) {
...
func Norm(a Matrix, norm float32) float32 {
	...
	if m, ok := m.(Normer); ok {
		if trans {
			switch norm {
			case 1:
				norm = float32(math.Inf(1))
			case float32(math.Inf(1)):
				norm = 1
			}
		}
		return m.Norm(norm)
	...
	case 1:
		...
		for j := 0; j < c; j++ {
			var sum float32
			for i := 0; i < r; i++ {
				sum += float32(math.Abs(float64(a.At(i, j))))
			}
			if sum > max {
				max = sum
			}
...
// normLapack converts the float32 norm input in Norm to a lapack.MatrixNorm.
// func normLapack(norm float32, aTrans bool) lapack.MatrixNorm {
// 	switch norm {
// 	case 1:
...
// use returns a float32 slice with l elements, using f if it
...
// useZeroed returns a float32 slice with l elements, using f if it
